// Using pure HTML templates instead of React for server-side rendering
      ...
      <div class="worksheet">
        <div class="header">
          ...
          <div class="header-right">
            <img class="logo" src="file://${path.join(__dirname, '../../public/images/Logo6.png')}" alt="PracticeGenius Logo">
          </div>
        </div>
...
/**
 * Renders worksheet items
 * @param {Array} items - Array of worksheet items
 * @param {Boolean} showAnswerBoxes - Whether to show answer boxes
...
/**
 * Renders assets for an item
 * @param {Array} assets - Array of assets
 * @returns {String} - HTML string for the assets
 */
const renderAssets = (assets) => {
  // Group assets by arrangement
  const assetsByArrangement = assets.reduce((acc, asset) => {
    const arrangement = asset.arrangement || 'row';
    if (!acc[arrangement]) {
      acc[arrangement] = [];
    }
    acc[arrangement].push(asset);
    return acc;
  }, {});
  
  // Render each arrangement group
  return Object.entries(assetsByArrangement).map(([arrangement, assetsGroup]) => {
    const assetsHtml = assetsGroup.map(asset => {
      // Repeat asset based on count
      const count = asset.count || 1;
      const assetHtml = Array(count).fill().map(() => {
    ...
    return `<div class="assets-container asset-${arrangement}">${assetsHtml}</div>`;
  }).join('');
};
 ...
 * @returns {String} - HTML string for the asset
 */
  ...
  switch (asset.type) {
    case 'icon':
      return `<div class="asset" style="width: ${sizeMap[size]}; height: ${sizeMap[size]};">
        <img src="file://${path.join(__dirname, `../../public/images/icons/${asset.name}.svg`)}" 
             alt="${asset.name}" 
...
/**
 * Renders an SVG shape
 * @param {String} shapeName - Name of the shape
 * @param {String} color - Color of the shape
 * @returns {String} - SVG path for the shape
 */
